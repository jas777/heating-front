  ...
  return (
    <div className={`w-48 select-none flex flex-col shadow-lg ${props.className}`}>
      <button onClick={toggle}
              className={`flex rounded-t-lg justify-center w-full heater-mode-info ${isAuto ? 'bg-violet-500' : 'bg-red-600'}`}>
        {isAuto ? 'AUTO' : 'MANUAL'}
      </button>
      ...
      <div className='flex bg-gray-200 w-full'>
        <button
          className='w-1/2 bg-red-600 text-white py-1 hover:bg-red-700 transition-all duration-200 font-bold'
          onClick={disable}
        >Wyłącz
        </button>
        <button
          className='w-1/2 bg-lime-500 text-white py-1 hover:bg-green-600 transition-all duration-200 font-bold'
          onClick={enable}
        >Włącz
        </button>
      </div>
